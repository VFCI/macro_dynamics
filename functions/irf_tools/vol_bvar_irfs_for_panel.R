impulseplots_panel <- function(ir, ## array of impulse response objects, nvar x nshock x nperiod
                        ...
                        xtick = c(1,5,10,15,20), ## where to put xticks; default, every 4 quarters
  ...
  for (j in findvar){ ## responses
      ...
      if(plottitle == TRUE)
          mtext(ptitle, side = 3, line = 0, cex = 1)
      
      axis(side = 1,cex.axis = 1.25,las = 1,at =xtick) #adds quarters 1-t to the x-axis
